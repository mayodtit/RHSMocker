...
class Mailcheck2 < Mailcheck
  ...
  def suggest(email)
    if email_parts = split_email(email.downcase)
      @threshold = 3
      suggest_for_known_address(email_parts)
    else
      suggest_for_unknown_address(email.downcase)
    end
  end
  ...
  def suggest_for_unknown_address(email)
    ...
    (1...len).each do |i|
      ...
      if result[:min_dist] && (result[:min_dist] + i * 0.05 < match[:dist])
    ...
    if match[:domain] 
      { 
        suggestion: { 
          ...
          domain: match[:domain], 
          full: "#{match[:address]}@#{match[:domain]}" 
        }
      }
    else
      nil
  ...
  def suggest_for_known_address(email_parts)
    ...
    @threshold = 4 if email_parts[:top_level_domain].blank?
    ...
    if closest_domain && closest_domain != email_parts[:domain]
      ...
      return { 
        suggestion: { 
          ...
          full: "#{email_parts[:address]}@#{closest_domain}" 
        }
      }
    else
      ...
      return { 
        suggestion: { 
          ...
          full: email_parts[:address] + '@' + email_parts[:domain] + '.com' 
        }
      } if email_parts[:top_level_domain].blank?
