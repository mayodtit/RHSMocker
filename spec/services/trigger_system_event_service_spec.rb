...
describe TriggerSystemEventService do
  describe '#call' do
    ...
    context 'for a task SystemEvent' do
      let!(:system_action_template) { create(:system_action_template, :task) }
      let!(:system_event_template) { system_action_template.system_event_template }
      let!(:system_event) { create(:system_event, user: user, system_event_template: system_event_template) }

      it 'creates a task for the user' do
        expect{ described_class.new(system_event: system_event).call }.to change(Task, :count).by(1)
        task = Task.last
        expect(task.task_template).to eq(system_action_template.unversioned_resource)
        expect(task.creator).to eq(Member.robot)
      end
      ...
      it 'creates a system action' do
        ...
        expect(action.system_event).to eq(system_event)
        expect(action.system_action_template).to eq(system_action_template)
        expect(action.result).to eq(Task.last)
      end
    end
  end
end
