...
describe TaskTemplate do
  ...
  describe '#create_deep_copy!' do
    ...
    let(:origin_task_template_attributes) { origin_task_template.attributes.slice(*%w(name title description time_estimate priority service_ordinal queue)) }
    ...
    it 'creates a deep copy including nested templates' do
      ...
      new_task_template_attributes = new_task_template.attributes.slice(*%w(name title description time_estimate priority service_ordinal queue))
