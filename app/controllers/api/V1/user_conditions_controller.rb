class Api::V1::UserConditionsController < Api::V1::ABaseController
  ...
  def index
    index_resource(@user.user_conditions, :user_diseases) and return if disease_path?
    index_resource(@user.user_conditions)
  end

  def show
    show_resource(@user_condition, :user_disease) and return if disease_path?
    show_resource(@user_condition)
  end

  def create
    create_resource(@user.user_conditions, params[:user_disease], :user_disease) and return if disease_path?
    create_resource(@user.user_conditions, params[:user_condition])
  end

  def update
    update_resource(@user_condition, params[:user_disease], :user_disease) and return if disease_path?
