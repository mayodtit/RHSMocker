    ...
    if (relative_days == 0) && reference_event.nil?
      ...
      if relative_hours > 0
        if (Time.now + relative_hours.hours + 1.hour).business_time?
          if (Time.now + relative_hours.hours) > 1.business_day.after(reference_time.pacific.eight_oclock) && (Time.now + relative_hours.hours) < 1.business_day.after(reference_time.pacific.nine_oclock)
            ...
            Time.now + relative_hours.hours + 1.hours
          else
