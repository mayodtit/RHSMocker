...
class PusherJob
  def push_content(user_id)
    ...
    PusherModule.broadcast(user.pusher_id, 'newcontent', content.id, content.contentsType)
  end
  ...
  def push_status(user_id, user_reading_id, status)
    ...
    PusherModule.broadcast(user.pusher_id, status, user_reading.content_id, user_reading.contentsType)
  end
end
